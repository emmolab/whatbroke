...
def main() -> None:
    ...
    for name, fn in checks.items():
        try:
            ...
            results.append(Result(name, "CRIT", "check raised an exception",
                                  [str(exc)]))
    ...
    for r in results:
        c = _color(r.status)
        print(f"{Colors.BOLD}{r.name}{Colors.END}: {c}{r.status}{Colors.END}"
              f"  {r.message}")
        if args.verbose:
            for d in r.details:
    ...
    c = _color(worst)
    print(f"{Colors.BOLD}Overall:{Colors.END} {c}{worst}{Colors.END}")
    sys.exit(SEVERITY[worst])
