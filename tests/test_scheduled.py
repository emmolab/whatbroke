...
class ScheduledCheckTests(unittest.TestCase):
    def test_user_cron_parser_ignores_env_comments_and_valid_macros(self):
        ...
        self.assertIsNone(scheduled._user_cron_issue_from_line("alice", "CRON_TZ = UTC"))
    ...
    def test_system_cron_parser_ignores_env_and_comments(self):
        ...
        self.assertIsNone(scheduled._system_cron_issue_from_line("/etc/crontab", 4, "PATH = /usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin"))
        self.assertIsNone(scheduled._system_cron_issue_from_line("/etc/crontab", 5, "*/5 * * * * root /usr/local/bin/check"))
        self.assertIsNone(scheduled._system_cron_issue_from_line("/etc/crontab", 6, "@daily root /usr/local/bin/rotate"))
